...
class TestDgAPIRequestBase:
    ...
    def test_player_skill_ratings(self, player_skill_ratings):
        assert sorted(['last_updated', 'players']) == sorted(list(player_skill_ratings.keys()))
        
    def test_detailed_approach_skill(self, detailed_approach_skill):
        assert sorted(['time_period', 'last_updated', 'data'])== sorted(list(detailed_approach_skill.keys()))
    
    def test_live_model_predictions(self, live_model_predictions):
        assert sorted(['info', 'data']) == sorted(list(live_model_predictions.keys()))
